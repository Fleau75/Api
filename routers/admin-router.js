...
exports.router = (() => {
    ...
    router
        ...
        router
            .route("/signin/")
            .post(checkAuthPayload, checkEmailPayload, signinResponse);

    // perte du mot de passe
    router.route("/recovery-password/").post(async (req, res) => {
        try {
    ...
    router.route("/new-password/").post(async (req, res) => {
        try {
            ...
            if (!recoveryPassword) {
                return res
                    .status(400)
                    .json({ message: "Lien de récupération invalide." });
            }
        ...
        router
            ...
            .post(async (req, res) => {
                try {
                    ...
                    console.error(
                        "Erreur lors de l'ajout d'ingrédients:",
                        error
                    );
                    res.status(500).json({
                        message: "Erreur lors de l'ajout d'ingrédients.",
                    });
                }
            })
            .get(async (req, res) => {
    ...
    router.route("/ingredients/:index").delete(async (req, res) => {
        try {
            const { index } = req.params;
            const userId = req.user._id;

            // Supprime l'ingrédient a l'index spécifié

            await User.findByIdAndUpdate(userId, {
                $pull: { ingredients: { _id: index } },
            });
